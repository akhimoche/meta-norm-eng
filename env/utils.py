...
from meltingpot.utils.substrates import substrate as utils_substrate
...
class MeltingPotEnv(multi_agent_env.MultiAgentEnv):
  ...
  def __init__(self, env: dmlab2d.Environment):
    ...
    self.observation_space = self._convert_spaces_tuple_to_dict(
        ...
        # remove_world_observations=False)
  ...
  def step(self, action_dict):
    """See base class."""
    actions = [action_dict[agent_id] for agent_id in action_dict.keys()]
...
def scenario_creator(scenario_name):
  factory = meltingpot.scenario.get_factory(scenario_name)
  base_env = factory.build()
  scaled = 1
  base_env = DownSamplingSubstrateWrapper(base_env, scaled)
  env = MeltingPotEnv(base_env)
...
class DownSamplingSubstrateWrapper(observables.ObservableLab2dWrapper):
    ...
    def __init__(self, utils_substrate: utils_substrate.Substrate, scaled):
        super().__init__(utils_substrate)
        self._scaled = scaled
